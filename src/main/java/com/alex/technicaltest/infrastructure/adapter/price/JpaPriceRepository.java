...
public interface JpaPriceRepository extends JpaRepository<PriceEntity, Long> {
    @Query("""
        SELECT e FROM PriceEntity e WHERE e.product.id = :productId
    ...
    @Query("""
        SELECT NEW com.alex.technicaltest.domain.dto.outbound.PriceResponseDto(
            e.product.id, e.brand.id, e.priceList, e.startDate, e.endDate, e.price, e.priority) 
        FROM PriceEntity e WHERE e.product.id = :productId
        AND e.brand.id = :brandId AND e.startDate <= :applicationDate AND e.endDate >= :applicationDate
            """)
